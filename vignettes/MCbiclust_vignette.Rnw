...
\section{Example of a single run}
\subsection{Loading Cancer Cell Line Encyclopedia (CCLE) and MitoCarta data sets}

For this example analysis we will be seeking to find biclusters related to mitochondrial function in the cancer cell line encyclopedia \cite{barretina2012cancer}. For this two datasets are needed, both of which are available on the MCbiclust package. The first in CCLE$\_$small that contains a subset of the gene expression values found in the entire CCLE data set (the full dataset is avaliable at \url{https://portals.broadinstitute.org/ccle/home}, the second, Mitochondrial$\_$genes, is a list of mitochondrial genes that can be found from MitoCarta1.0 \cite{pagliarini2008mitochondrial}.
...
It is a simple procedure to create a new matrix CCLE.mito only containing the mitochondrial genes. While there are $1023$ known mitochondrial genes, not all of these are measured in CCLE$\_$data. The data frame CCLE$\_$data also contains columns with the probe and gene names these are removed to form the data frame CCLE.data.
...
\subsection{Finding a bicluster seed}
...
For reproducibility \Rfunction{set.seed} has been used to set R's pseudo-random number generator. It should also be noted that the for `gem` the data matrix can not contain all the genes, since \Rfunction{FindSeed} involves the calculation of correlation matrices which are not computationally efficient to compute if they involve greater than ~1000 genes.
...
\Rfunction{FindSeed} has one more additional options, `initial.seed` allows the user to specify the initial subsample to be tested, by default the initial sample subset is randomly chosen. 

There is a function \Rfunction{CorScoreCalc} that can calculate the correlation score $\alpha_I^J$ directly, in general however you should not need to use it, unless you wish to manually check the chosen seed is an improvement on one that is randomly generated.
...
The results of \Rfunction{FindSeed} can also be visualised by examining the associated correlation matrix, and viewing the result as a heatmap. Again it is easy to see the difference between the random subsample and the one outputted from \Rfunction{FindSeed}.
...
\Rfunction{heatmap.2} is a function from the `gplots` R package. 

\subsection{Selecting highly correlated genes}
As can be clearly seen from the heat map, not all the mitochondrial genes are equally strongly correlated to each other. There is a function in `MCbiclust` which automatically selects those genes that are most strongly associated with the pattern. This function is \Rfunction{HclustGenesHiCor} and it works by using hierarchical clustering to select the genes into n different groups, and then discarding any of these groups that fail to have a correlation score greater than the correlation score from all the genes together.
<<fig=TRUE,  width=5, height=4>>=
...
\subsection{Calculate the correlation vector}
...
\begin{enumerate}
\item The pattern is summarised  by finding a subset of genes which all strongly correlate with each other, and calculating their average expression value. This is done by clustering the genes using hierarchical clustering and selecting the best group judged by that groups correlation score multiplied by the square root of the number of genes. This multiplication is done to remove the bias of selecting a group containing very few genes.

\item The correlation vector is calculated by finding the correlation of every gene to the average expression value of the chosen best group.

\end{enumerate}
This process is all encapsulated in the function `CVEval` which takes 4 arguements. `gem.part` is the gene expression matrix for the chosen gene set of interest, e.g. mitochondrial genes, `gem.all` is the entire gene expression matrix, `seed` is the output from \Rfunction{FindSeed} and `splits` is the number of groups to split the chosen gene set into in order to select the best group.
...
This is achieved with the \Rfunction{GOEnrichmentAnalysis} function which takes three inputs:

\begin{enumerate}
\item `gene.names`: The names of the genes in standard format.

\item `gene.values`: The correlation vector.

\item `sig.rate`: The value below which adjusted p-values are decided to be significant.
\end{enumerate}
The output is a table with 7 columns:
\begin{enumerate}
  \item  `GOID`: ID for GO term.
\item  `TERM`: Name of GO term.
\item  `num.genes`: Number of genes in GO term.
\item  `g.in.genelist`: Number of genes in GO term that were measured in the gene expression matrix.
\item  `adj.p.value`: Adjusted p-value from Mann-Whitney test.
\item  `CV.av.value`: Average value of CV for genes in GO term.
\item  `phenotype`: +1 if `CV.av.value` is greater than the overall CV average, -1 if the `CV.av.value` is less then the overall CV average.
\end{enumerate}
...
\subsection{Sample ordering}
Already all the genes in the data set have had the correlation calculated to the pattern found. One more task that can be readily done is to order the samples according to the strength of correlation. Function \Rfunction{FindSeed} found the initial $n$ samples that had a very strong correlation with the gene set of interest, the $n+1$ sample is to be selected as that sample which best maintains the correlation strength, this process can be simply repeated until all or the desired number of samples are ordered.

\Rfunction{SampleSort} is the function in `MCbiclust` that completes this procedure, it has $4$ main inputs:

\begin{enumerate}
\item `gem`: the gene expression matrix with all the samples and the gene set of interest.
\item `seed`: the initial subsample found with `FindSeed`.
\item ` num.cores`: Used for setting the number of cores used in calculation, important to use to increase the speed of what is a slow calculation.
\item `sort.length`: Sets the number of samples to be ordered.
\end{enumerate}
...
Note as before that these are long calculations, the precalculated orderings are however given in the `Vignette$\_$sort` data file.
...
\Rfunction{PC1VecFun} is a function that calculates the PC1 values for all sorted samples. It takes three inputs:

\begin{enumerate}
\item `top.gem` is the gene expression matrix with only the most highly correlated genes but with all the sample data.
\item `seed.sort` is the sorting of the data samples found with function \Rfunction{SampleSort}
\item `n` is the number of samples used for initially calculating the weighting of PC1. If set to $10$, the first $10$ samples are used to calculate the weighting of PC1 and then the value of PC1 is calculated for all samples in the ordering.
\end{enumerate}
...
\subsection{Thresholding the bicluster and aligning PC1}
So far MCbiclust outputs a ranked list of genes and samples. In many cases it is however necessary to determine which genes and samples are within the bicluster and which are not. This is done with the \Rfunction{ThresholdBic} function, which takes $4$ arguements:
\begin{enumerate}
\item `cor.vec`: The correlation vector, output of `CVeval`.
\item `sort.order`: The sorted samples, output of `SampleSort`.
\item `pc1`: The PC1 vector, output of `PC1VecFun`
\item `samp.sig`: A numeric value between 0 and 1 that detemines the number of samples in the bicluster.
\end{enumerate}
...
This is done with function \Rfunction{PC1Align} which if necessary times the pc1.vec by -1 to ensure that the correlation vector and PC1 vector are "aligned".
...
One final additional thing that can be done is to classify the samples into belonging to the bicluster or not, and additionally whether a sample belongs to the Upper or Lower fork. This can be done with the function \Rfunction{ForkClassifier}
...
\subsection{Comparing results with sample data}
This section will deal with an addition data sets both of which are available in the MCbiclust package.
\begin{itemize}
  \item CCLE sample information, a data set containing information for every sample in the data set, including gender of the patient the cell line was derived from, as well as the primary site it came from.
\end{itemize}
...
\subsubsection{CCLE sample data}
...
In this case some samples have an additional "X" not present in some CCLE$\_$samples data so it is necessary to add it for consistency.
...
\section{Dealing with multiple runs}
...
Here two clusters were found, and we can visualise this pattern (and any additional others found) with the function \Rfunction{CVPlot}, which highlights a chosen gene set, in this case the mitochondrial genes.
...
CCLE.samp.multi.sort[[1]] <- SampleSort(gem = multi.prep[[1]][[1]],
                                        seed = multi.prep[[2]][[1]],
                                        num.cores = 4)
CCLE.samp.multi.sort[[2]] <- SampleSort(gem = multi.prep[[1]][[1]],
                                        seed = multi.prep[[2]][[1]],
                                        num.cores = 4)   
@
...
data("Vignette_sort")
CCLE.samp.multi.sort <- list()
CCLE.samp.multi.sort[[1]] <- Vignette_sort[[2]][[1]]
CCLE.samp.multi.sort[[2]] <- Vignette_sort[[2]][[2]]
@
